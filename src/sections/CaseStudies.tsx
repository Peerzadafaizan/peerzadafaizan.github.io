...
function CaseStudyCard({ item }: { item: CaseStudy }) {
  ...
  return (
    <article
      ...
      className={`@container rounded-lg border border-line bg-surface p-5 shadow-sm sm:p-7 lg:p-8 ${cardHover}`}
    >
      ...
      <div className="grid gap-6 @3xl:grid-cols-[minmax(0,15rem)_minmax(0,1fr)] @3xl:grid-rows-[auto_1fr] @3xl:gap-x-10 @3xl:gap-y-6">
        <div data-reveal-inner className="@3xl:col-start-1 @3xl:row-start-1">
        ...
        <dl data-reveal-inner className="grid gap-px overflow-hidden rounded-md border border-line bg-line @xl:grid-cols-2 @3xl:col-start-2 @3xl:row-span-2 @3xl:row-start-1">
          {BLOCKS.map((key) => (
        ...
        <ul data-reveal-inner="fade" className="flex flex-wrap gap-2 @3xl:col-start-1 @3xl:row-start-2 @3xl:self-end">
          {item.tags.map((tag) => (
 ...
 * Motion: each card rises in as it scrolls into view (cards entering together are staggered);
 * on desktop its title, blocks and tags follow in sequence. Cards are not clickable, so the only
 * hover feedback is a slightly firmer border.
 */
  ...
  return (
    ...
    >
      <div className="container-page">
        ...
        <p data-reveal="fade" className="mt-8 max-w-[46rem] border-t border-dashed border-line pt-5 text-small text-ink-subtle italic">
          {caseStudies.note}
        </p>
      </div>
    </section>
