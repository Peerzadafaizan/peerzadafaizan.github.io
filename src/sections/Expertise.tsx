 ...
 * and src/content/tools.ts. Static: no ratings, no percentages, no motion.
 */
      ...
      <div className="container-page">
        ...
        <ul className="mt-10 grid gap-4 sm:mt-12 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
          {expertise.areas.map((area) => (
            <li key={area.title} className="flex flex-col rounded-lg border border-line bg-surface p-5 sm:p-6">
        ...
        >
          <p className={eyebrowClass}>{toolsHeading.eyebrow}</p>
          <h3
          ...
          <ul className="mt-8 grid gap-px overflow-hidden rounded-lg border border-line bg-line sm:grid-cols-2 lg:grid-cols-3">
            {tools.items.map((tool) => (
              <li key={tool.name} className="flex items-start gap-4 bg-surface p-5">
